...
public class AddressService {
	...
	public List<AddressDTO> findAll() {
		...
		return addresss.stream().map((address) -> mapToDTO(address, new AddressDTO())).collect(Collectors.toList());
	}

	public AddressDTO get(final String id) {
		return addressRepository.findById(id).map(address -> mapToDTO(address, new AddressDTO()))
	...
	public void update(final String id, final AddressDTO addressDTO) {
		final Address address = addressRepository.findById(id).orElseThrow(() -> new NotFoundException());
		mapToEntity(addressDTO, address);
	...
	private Address mapToEntity(final AddressDTO addressDTO, final Address address) {
		...
		final User user = addressDTO.getUser() == null ? null
				: userRepository.findById(addressDTO.getUser())
