...
public class OrderService {
	...
	public List<OrderDTO> findAll() {
		...
		return orders.stream().map((order) -> mapToDTO(order, new OrderDTO())).collect(Collectors.toList());
	}

	public OrderDTO get(final String id) {
		return orderRepository.findById(id).map(order -> mapToDTO(order, new OrderDTO()))
	...
	public void update(final String id, final OrderDTO orderDTO) {
		final Order order = orderRepository.findById(id).orElseThrow(() -> new NotFoundException());
		mapToEntity(orderDTO, order);
	...
	private Order mapToEntity(final OrderDTO orderDTO, final Order order) {
		...
		final User user = orderDTO.getUser() == null ? null
				: userRepository.findById(orderDTO.getUser())
						.orElseThrow(() -> new NotFoundException("user not found"));
		order.setUser(user);
		final Product product = orderDTO.getProduct() == null ? null
				: productRepository.findById(orderDTO.getProduct())
						.orElseThrow(() -> new NotFoundException("product not found"));
		order.setProduct(product);
		final Address address = orderDTO.getAddress() == null ? null
				: addressRepository.findById(orderDTO.getAddress())
